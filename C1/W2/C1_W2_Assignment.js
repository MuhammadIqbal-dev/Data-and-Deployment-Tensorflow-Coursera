...
async function train(model, data) {
    ...
    const [trainXs, trainYs] = tf.tidy(()=>{
        const dt = data.nextTrainBatch(TRAIN_DATA_SIZE);
        return [dt.xs.reshape([TRAIN_DATA_SIZE, 28, 28, 1]),dt.labels]
    });
    ...
    const [testXs, testYs] = tf.tidy(()=>{
        const dt = data.nextTestBatch(TEST_DATA_SIZE);
        return [dt.xs.reshape([TEST_DATA_SIZE, 28, 28, 1]),dt.labels]
    });
